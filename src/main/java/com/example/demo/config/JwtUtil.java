...
public class JwtUtil {
	...
	public String generateToken(String email, String role) {
		log.info("JwtUtil initialized with expiration time: " + EXPIRATION_TIME + " minutes");
		String token = Jwts.builder().setSubject("User Details").setHeaderParam("typ", "JWT")
				.claim("data", new Payload(email, role)).setIssuedAt(new Date(System.currentTimeMillis()))
	...
	public Boolean isTokenExpired(String token) {
		try {
			...
			Claims claims = Jwts.parser().verifyWith(SECRET_KEY).build().parseSignedClaims(token).getBody();

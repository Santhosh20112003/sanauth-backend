	...
	public ResponseEntity<Map<String, String>> verifyEmail(@RequestBody EmailAndOtp request) {
	    ...
	    if (email == null || otp == null) {
	        return ResponseEntity
	                .badRequest()
	                .body(Map.of("error", "Email and OTP must not be null"));
	    }

	    // Fetch latest unused OTP for verification
	    Optional<UserOtp> otpOptional = otpRepository.findTopByEmailAndTypeAndUsedOrderByCreatedAtDesc(email, "VERIFICATION", false);
	    ...
	    if (user.isEmailVerified()) {
	        return ResponseEntity
	                .ok(Map.of("message", "Email is already verified"));
	    }
	    ...
	    return ResponseEntity
	            .ok(Map.of("message", "Email verified successfully"));
	}
	...
	public ResponseEntity<Map<String, String>> resendOtp(@RequestBody MailModal request) {
		...
		if (userOptional.isPresent()) {
			...
			if (!user.isEmailVerified()) {
				...
				if (emailSent) {
					return new ResponseEntity<>(Map.of("message", "Verification email sent successfully"), HttpStatus.OK);
				} else {
					return new ResponseEntity<>(Map.of("error", "Failed to send verification email"), HttpStatus.INTERNAL_SERVER_ERROR);
				}
			} else {
				return new ResponseEntity<>(Map.of("message", "Email is already verified"), HttpStatus.OK);
			}
		} else {
			return new ResponseEntity<>(Map.of("error", "User not found with the provided email"), HttpStatus.NOT_FOUND);
		}
	}
	...
	public ResponseEntity<Map<String, String>> forgotPassword(@RequestBody MailModal request) {
		...
		if (userOptional.isPresent()) {
			...
			if (emailSent) {
				...
				return new ResponseEntity<>(Map.of("error", "Failed to send reset password email"), HttpStatus.INTERNAL_SERVER_ERROR);
			}
		} else {
			return new ResponseEntity<>(Map.of("error", "User not found with the provided email"), HttpStatus.NOT_FOUND);
		}
	...
	public ResponseEntity<Map<String, String>> resetPassword(@RequestBody EmailOtpAndPassword request) {
	    ...
	    if (email == null || otp == null || newPassword == null || newPassword.isBlank()) {
	        return ResponseEntity.badRequest().body(
	            Map.of("error", "Email, OTP, and password must not be null or empty"));
	    }
	    ...
	    try {
	        ...
	        Optional<UserOtp> otpOptional = otpRepository
	                .findTopByEmailAndTypeAndUsedOrderByCreatedAtDesc(email, "RESET_PASSWORD", false);
